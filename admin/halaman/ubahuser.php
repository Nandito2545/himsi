...
<form method="post" action="">
    ...
    <?php
        if (@$_POST['simpan']){
            ...
            if(empty($password)){
                mysqli_query($k, "UPDATE tb_user SET nama_user = '$nama', alamat='$alamat', telepon = '$tlp', username ='$user' WHERE id_user = '$id'")or die(mysqli_error($k).mysqli_errno($k));
            }
            else{
                mysqli_query($k, "UPDATE tb_user SET nama_user = '$nama', alamat='$alamat', telepon = '$tlp', username ='$user', password = '$password' WHERE id_user = '$id'");
            }
